...
public class ForwardResource implements Resource {
	...
	public Response process(Request request, DatabaseStorage dbInstance) {
		...
		if (nextNode != null) {
			...
			if(fwd != null){
			HeartbeatData nextNodeData = HeartbeatManager.getInstance().getHeartbeatData(nextNode);
			
			ChannelFuture cf = nextNodeData.getChannel().write(fwd);  // Forwarded the request
			}
	...
	private String determineForwardNode(Request request) {
		...
		for (NodeDesc nd : cfg.getNearest().getNearestNodes().values()) {   // TODO we have to loop through all the nodes in the routing path not just first node
