		...
		while (true) {
			...
			try {
				...
				if (req.hasBeat()) {
					...
					} else if (n.getAction().getNumber() == Action.NODELEAVE_VALUE) {    // Can process and put the response message on the outbound queue
						// not left network gracefully									// where it will be processed by outboudworkerthread	
